...
def test_add_tool_call_and_result():
    context = ChatContext()
    tool_call = context.add_tool_call("my_tool", {"key": "value"})
    context.add_tool_result(tool_call.id, {"result": "ok"})
    assert len(context.items) == 2
    assert isinstance(context.items[0], ToolCallItem)
    assert isinstance(context.items[1], ToolResultItem)


def test_messages_for_llm_includes_tool_calls():
    context = ChatContext()
    context.add_message("user", "Hello")
    tool_call = context.add_tool_call("my_tool", {"key": "value"})
    context.add_tool_result(tool_call.id, {"result": "ok"})
...
def test_messages_for_llm_excludes_events():
    context = ChatContext()
    context.add_message("user", "Hello")
    context.add_event("some_event", {"data": "value"})
...
def test_compact_respects_max_items():
...
def test_auto_compact_triggered_on_append():
    context = ChatContext(max_context_items=3)
    for i in range(10):
        context.add_message("user", f"message {i}")
...
def test_resume_restores_items(tmp_path):
    store = SessionStore(db_path=tmp_path / "test.db")
